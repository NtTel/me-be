...
import { 
  ...
  Flag, LogOut, Menu, X, ArrowLeft, Database, Gamepad2, 
      ...
      >
        ...
        <div className="h-16 flex items-center px-6 border-b border-gray-100 justify-between md:justify-start">
          ...
          <button 
            ...
            className="md:hidden text-gray-500 hover:text-gray-900 p-1 rounded-full"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <nav className="p-4 space-y-1 overflow-y-auto max-h-[calc(100vh-8rem)]">
          {navItems
            .filter(item => item.roles.includes(currentUser.isAdmin ? 'admin' : 'expert'))
            .map(item => {
