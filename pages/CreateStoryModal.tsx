...
export const CreateStoryModal: React.FC<CreateStoryModalProps> = ({ currentUser, onClose, onSuccess }) => {
    ...
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center animate-fade-in p-4">
      ...
      <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl relative flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 p-4 flex flex-col items-center justify-center min-h-[400px] bg-gray-50 dark:bg-black/50 relative overflow-hidden">
          ...
          {error && (
            <div className="absolute top-4 left-4 right-4 bg-red-50 text-red-600 px-3 py-2 rounded-lg text-sm border border-red-200 z-20 text-center animate-shake">
              {error}
            </div>
          )}
          ...
          {!previewUrl ? (
            ...
            >
              <div className="w-20 h-20 bg-blue-100 dark:bg-blue-900/50 rounded-full flex items-center justify-center text-primary group-hover:scale-110 transition-transform duration-300">
                <ImageIcon size={40} />
              </div>
              <div className="text-center">
                <p className="font-bold text-gray-700 dark:text-gray-200 text-lg mb-1">Chọn ảnh khoảnh khắc</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Hỗ trợ JPG, PNG, WEBP</p>
              </div>
              <button className="bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-full text-sm font-bold shadow-sm border border-gray-200 dark:border-gray-700 mt-2 flex items-center gap-2 group-hover:bg-primary group-hover:text-white group-hover:border-primary transition-colors">
            ...
            <div className="relative w-full h-full rounded-xl overflow-hidden bg-black flex items-center justify-center animate-zoom-in">
               ...
               {!isUploading && (
                 <button 
                   ...
                   className="absolute top-3 right-3 bg-black/60 text-white p-2 rounded-full hover:bg-red-500 backdrop-blur-md transition-all shadow-lg"
                   title="Xóa ảnh này"
                 >
                   <Trash2 size={18} />
                 </button>
        ...
        <div className="p-4 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900">
           <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-bold text-gray-900 dark:text-white">Đăng bởi</span>
           ...
           <div className="flex gap-3">
             <button 
               onClick={onClose}
               disabled={isUploading}
               className="px-5 py-2.5 rounded-full font-bold text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800 transition-colors disabled:opacity-50"
             >
               Hủy
             </button>
             <button 
               ...
               className={`px-6 py-2.5 rounded-full font-bold text-white flex items-center gap-2 transition-all shadow-lg ${
                 !selectedFile || isUploading 
                   ? 'bg-gray-300 dark:bg-gray-700 cursor-not-allowed shadow-none' 
                   : 'bg-gradient-to-r from-primary to-blue-600 hover:opacity-90 active:scale-95 shadow-primary/30'
               }`}
             >
               {isUploading ? (
                 <>
