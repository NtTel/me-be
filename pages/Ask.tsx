...
export enum AskCategoryKey {
  ...
  Age1_3 = "1-3 tuổi",
  Psychology = "Tâm lý",
  Education = "Giáo dục sớm",
  Family = "Gia đình",
  Default = "Default",

}
const CATEGORY_CONFIG: Record<string, { icon: any, color: string, bg: string, border: string }> = {
  [AskCategoryKey.Pregnant]: { icon: Baby, color: "text-pink-600 dark:text-pink-400", bg: "bg-pink-50 dark:bg-pink-900/20", border: "border-pink-100 dark:border-pink-900/30" },
  [AskCategoryKey.Nutrition]: { icon: Utensils, color: "text-green-600 dark:text-green-400", bg: "bg-green-50 dark:bg-green-900/20", border: "border-green-100 dark:border-green-900/30" },
  [AskCategoryKey.Health]: { icon: Stethoscope, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-50 dark:bg-blue-900/20", border: "border-blue-100 dark:border-blue-900/30" },
  [AskCategoryKey.Age0_1]: { icon: Smile, color: "text-indigo-600 dark:text-indigo-400", bg: "bg-indigo-50 dark:bg-indigo-900/20", border: "border-indigo-100 dark:border-indigo-900/30" },
  [AskCategoryKey.Age1_3]: { icon: Smile, color: "text-indigo-600 dark:text-indigo-400", bg: "bg-indigo-50 dark:bg-indigo-900/20", border: "border-indigo-100 dark:border-indigo-900/30" },
  [AskCategoryKey.Psychology]: { icon: Brain, color: "text-purple-600 dark:text-purple-400", bg: "bg-purple-50 dark:bg-purple-900/20", border: "border-purple-100 dark:border-purple-900/30" },
  [AskCategoryKey.Education]: { icon: BookOpen, color: "text-yellow-600 dark:text-yellow-400", bg: "bg-yellow-50 dark:bg-yellow-900/20", border: "border-yellow-100 dark:border-yellow-900/30" },
  [AskCategoryKey.Family]: { icon: Users, color: "text-teal-600 dark:text-teal-400", bg: "bg-teal-50 dark:bg-teal-900/20", border: "border-teal-100 dark:border-teal-900/30" },
...
const getCategoryLabel = (catName: string, t: (key: string) => string) => {
  // если категория известна в конфиге и для неё есть перевод – берём перевод
  if (CATEGORY_CONFIG[catName]) {
    return t(`ask.categories.${catName}`);
  }
  // иначе это пользовательская категория – показываем как есть
  return catName;
};
...
const ToastContainer = ({ toasts }: { toasts: ToastMessage[] }) => (
  <div className="fixed top-4 left-0 right-0 z-[100] flex flex-col items-center gap-2 pointer-events-none px-4">
    {toasts.map(t => (
      <div key={t.id} className={`flex items-center gap-2 px-4 py-3 rounded-xl shadow-lg shadow-black/5 animate-slide-down backdrop-blur-md max-w-sm w-full pointer-events-auto border ${t.type === 'error' ? 'bg-red-50/90 dark:bg-red-900/80 text-red-600 dark:text-red-200 border-red-100 dark:border-red-800' :
...
export const Ask: React.FC<AskProps> = ({
  ...
  const handleAiSuggest = async () => {
    ...
    try {
      ...
      showToast(t("ask.aiBusyError"), "error");
    } finally {
  ...
  const handleAiContent = async () => {
    if (!title || title.length < 5) {
      showToast(t("ask.aiNeedTitleError"), "error");
      return;
    }
    if (content.length > 50) {
      if (!confirm(t("ask.aiOverwriteConfirm"))) return;
    }
    setIsGeneratingContent(true);
    try {
      const aiContent = await generateQuestionContent(title);
      setContent(aiContent);
      showToast(t("ask.aiWriteDone"), "success");
    } catch (error: any) {
      if (error.message?.includes('429')) {
        showToast(t("ask.aiOverloaded"), "error");
      } else {
        showToast(t("ask.aiError"), "error");
      }
    } finally {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (attachments.length + filesArray.length > 3) {
        showToast(t("ask.imageLimitError"), "error");
        return;
      }
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-dark-card border-t border-gray-100 dark:border-dark-border px-4 py-3 pb-safe-bottom z-40 shadow-[0_-5px_25px_rgba(0,0,0,0.03)] dark:shadow-none transition-colors">
        <div className="max-w-3xl mx-auto flex flex-col gap-3">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            >
              {isSubmitting ? <Loader2 size={22} className="animate-spin" /> : <>{t("ask.submittingButton")}<Send size={20} /></>}
            </button>
          </div>
        </div>
      </div>
      ...
      {showCategorySheet && (
        <div className="fixed inset-0 z-[60] flex flex-col justify-end">
          ...
          <div className="bg-white dark:bg-dark-card rounded-t-[2rem] p-6 pb-safe-bottom relative z-10 animate-slide-up shadow-2xl max-h-[85vh] overflow-y-auto transition-colors">
            ...
            <h3 className="font-bold text-xl text-gray-900 dark:text-white mb-6 text-center">{t("ask.categorySheetTitle")}</h3>
            ...
            <div className="flex gap-2 mb-6">
              <input
                ...
                placeholder={t("ask.customCategoryPlaceholder")}
              ...
              {allCategories.map(cat => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left transition-all active:scale-[0.98] flex items-center gap-4 ${isSelected ? `border-[#25A99C] bg-[#25A99C]/5 dark:bg-[#25A99C]/10 shadow-sm` : 'border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm hover:border-gray-300 dark:hover:border-slate-600'
                      }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <span className={`block font-bold text-base truncate ${isSelected ? 'text-[#25A99C]' : 'text-gray-800 dark:text-white'}`}>{getCategoryLabel(cat, t)}</span>
                      {isSelected && <span className="text-xs text-[#25A99C] font-medium flex items-center gap-1 mt-0.5"><Check size={12} /> {t("ask.selectedLabel")}</span>}
                    </div>
                  </button>
                );
              })}
