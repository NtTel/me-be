    ...
    <div className="space-y-4 animate-fade-in">
        ...
        <div className="pl-4 md:px-0 animate-slide-up">
           ...
           <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 pr-4 snap-x">
             ...
             {matchingUsers.map(user => (
               <Link 
                 to={`/profile/${user.username || user.id}`}   // <--- ĐÃ SỬA TẠI ĐÂY
                 key={user.id} 
                 className="snap-start flex-shrink-0 bg-white p-3 pr-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3 min-w-[160px] active:scale-95 transition-transform"
               >
                   ...
                   <div className="flex flex-col">
                       <span className="text-sm font-bold text-textDark truncate max-w-[100px]">
                         {user.name}
                       </span>
            ...
            <div className="bg-gradient-to-br from-primary to-[#26A69A] rounded-3xl p-6 text-white shadow-xl shadow-primary/20 relative overflow-hidden">
                ...
                <div className="relative z-10 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-blue-50 text-xs font-medium opacity-90 mb-3">
                          Chia sẻ kinh nghiệm: Góp phần tạo ra thay đổi tích cực cho cộng đồng.
                        </p>
                        <Link 
                          to="/expert-register" 
                          className="inline-flex items-center gap-1 bg-white/20 hover:bg-white/30 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs font-bold transition-all active:scale-95 border border-white/20"
                        >
                ...
                <div className="space-y-3 pt-2">
                    <div className="flex justify-between items-center px-1">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-bold text-textDark text-sm uppercase tracking-wide">
                              Kiến thức Chuyên gia
                            </h3>
                        </div>
                        <Link to="/blog" className="text-xs font-bold text-blue-500 hover:underline">
                          Xem tất cả
                        </Link>
                    </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 pr-4 snap-x -mx-4 px-4 md:mx-0 md:px-0">
                        {blogPosts.map(post => (
                            <Link 
                              to={`/blog/${post.slug}`} 
                              key={post.id} 
                              className="snap-start flex-shrink-0 w-64 bg-white rounded-2xl p-3 border border-gray-100 shadow-sm hover:shadow-md transition-all active:scale-95 flex flex-col"
                            >
                                <div className="aspect-[2/1] rounded-xl bg-gray-100 mb-3 overflow-hidden relative shrink-0">
                                    {post.coverImageUrl ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center text-3xl bg-gradient-to-br from-blue-50 to-purple-50">
                                          {post.iconEmoji || '📝'}
                                        </div>
                                    )}
                                </div>

                                <h4 className="font-bold text-sm text-textDark line-clamp-2 mb-1 leading-snug flex-1">
                                  {post.title}
                                </h4>

                                <div className="flex items-center gap-1 mt-auto pt-2">
                                    <span className="text-[10px] text-gray-400 bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100 line-clamp-1 max-w-[100px]">
                                      {post.authorName}
                                    </span>
            ...
            {documents.length > 0 && (
                <div className="space-y-3 pt-2">
                    <div className="flex justify-between items-center px-1">
                        ...
                        <Link to="/documents" className="text-xs font-bold text-green-500 hover:underline">
                          Xem tất cả
                        </Link>
                    </div>
                    ...
                    <div className="space-y-3">
                        {documents.map(doc => (
                            <Link 
                              ...
                              key={doc.id} 
                              className="flex items-center gap-4 bg-white p-3 rounded-2xl border border-gray-100 shadow-sm active:scale-[0.98] transition-transform group"
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-sm text-textDark line-clamp-1 group-hover:text-green-600 transition-colors">
                                      {doc.title}
                                    </h4>
      ...
      {!searchQuery && (
        <div className="pl-4 md:px-0 mt-6">
            <div className="flex items-center gap-1 mb-2">
                ...
                <span className="text-xs font-bold text-textGray uppercase tracking-wider">
                  Chủ đề
                </span>
            </div>
              ...
              <button 
                ...
                className={`snap-start flex-shrink-0 px-5 py-2.5 rounded-2xl text-sm font-bold whitespace-nowrap transition-all active:scale-95 
                  ${activeCategory === 'Tất cả' 
                    ? 'bg-textDark text-white shadow-lg shadow-gray-200' 
                    : 'bg-white text-textGray border border-gray-100 shadow-sm'}
                `}
              >
                Tất cả
              </button>
            ...
            <div className="flex bg-white p-1 rounded-xl border border-gray-100 shadow-sm">
                <button 
                  ...
                  className={`p-1.5 rounded-lg transition-all 
                    ${viewFilter === 'newest' ? 'bg-gray-100 text-textDark' : 'text-gray-400'}`}
                >
                  <Clock size={16} />
                </button>
                ...
                <button 
                  ...
                  className={`p-1.5 rounded-lg transition-all 
                    ${viewFilter === 'active' ? 'bg-orange-50 text-orange-500' : 'text-gray-400'}`}
                >
                  <Flame size={16} />
                </button>
                ...
                <button 
                  ...
                  className={`p-1.5 rounded-lg transition-all 
                    ${viewFilter === 'unanswered' ? 'bg-blue-50 text-blue-500' : 'text-gray-400'}`}
                >
      ...
      <div className="px-4 md:px-0 space-y-4 pb-10">
         ...
         {paginatedQuestions.map((q, index) => {
            const query = searchQuery.toLowerCase().trim();
            const matchedAnswer = searchQuery && q.answers.find(
            ...
            return (
              <React.Fragment key={q.id}>
                ...
                <div className="bg-white p-5 rounded-[1.5rem] shadow-[0_2px_15px_rgba(0,0,0,0.03)] border border-gray-100 active:scale-[0.98] transition-all relative overflow-hidden">
                    ...
                    {q.answers.length === 0 && (
                      <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-orange-100 to-transparent rounded-bl-full -mr-8 -mt-8"></div>
                    )}
                    ...
                    <div className="flex items-start justify-between mb-3 relative z-10">
                      ...
                      <span className="bg-gray-50 text-textGray text-[10px] font-bold px-2 py-1 rounded-lg border border-gray-100">
                        {q.category}
                      </span>
                    </div>

                    <h3 className="text-[16px] font-bold text-textDark mb-2 leading-snug line-clamp-2">
                      {q.title}
                    </h3>

                    <p className="text-textGray text-sm line-clamp-2 mb-3 font-normal">
                      {q.content}
                    </p>
                    ...
                    {matchedAnswer && (
                        <div className="bg-blue-50 rounded-xl p-3 mb-3 flex gap-2 border border-blue-100 mt-3">
                            ...
                            <div>
                                <p className="text-xs font-bold text-textDark mb-0.5">
                                  {matchedAnswer.author.name} đã trả lời:
                                </p>
                                <p className="text-xs text-textGray line-clamp-1 italic">
                                  "{matchedAnswer.content}"
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-50 mt-3">
                      <div className="flex items-center gap-4 text-xs font-bold text-gray-400">
                          <span className="flex items-center gap-1.5">
                            <Heart size={14} className={q.likes > 0 ? "text-red-500 fill-red-500" : ""} /> 
                            {q.likes}
                          </span>
                          <span className="flex items-center gap-1.5">
                            <MessageCircle size={14} className={q.answers.length > 0 ? "text-blue-500 fill-blue-500" : ""} />
                            {q.answers.length}
                          </span>
                      </div>
