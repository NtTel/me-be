...
export const Profile: React.FC<ProfileProps> = ({ user, questions, onLogout, onOpenAuth }) => {
  ...
  const { userId } = useParams<{ userId: string }>(); // ID từ URL
  
  const [activeTab, setActiveTab] = useState<'overview' | 'questions'>('overview');
  
  // Dữ liệu của người ĐANG ĐƯỢC XEM (Target User)
  ...
  // Trạng thái: Mình có đang theo dõi người này không?
  const [isFollowing, setIsFollowing] = useState(false);

  // --- 1. LOGIC TỰ ĐỘNG CHUYỂN HƯỚNG URL ---
  // Nếu vào /profile mà không có ID -> Chuyển ngay sang /profile/ID-CUA-MINH
  useEffect(() => {
  ...
  // Xác định ID cần lấy dữ liệu (Ưu tiên ID trên URL, nếu không có thì lấy ID của mình)
  const targetId = userId || user?.id;
  const isViewingSelf = user && targetId === user.id;

  // --- 2. LẮNG NGHE DATA NGƯỜI ĐANG ĐƯỢC XEM (REAL-TIME) ---
  // Giúp cập nhật Avatar, Bio, và Số lượng Followers ngay lập tức khi có thay đổi
  useEffect(() => {
    if (!targetId) return;
  ...
  // --- 3. LẮNG NGHE DATA CỦA CHÍNH MÌNH (REAL-TIME) ---
  // Để biết danh sách "Đang theo dõi" của mình có chứa người kia không
  useEffect(() => {
    // Chỉ chạy khi: Đã đăng nhập + Không phải khách + Đang xem profile người khác
    if (user && !user.isGuest && targetId && targetId !== user.id) {
  ...
  const handleFollowToggle = async () => {
    ...
    try {
        if (isFollowing) {
            ...
            // Gửi thông báo
            await sendNotification(profileData.id, user, 'FOLLOW', 'đã bắt đầu theo dõi bạn.', `/profile/${user.id}`);
        }
        // Không cần set state thủ công, onSnapshot sẽ tự lo việc cập nhật UI
    } catch (e) {
        console.error("Follow error:", e);
        alert("Không thể thực hiện hành động. Vui lòng kiểm tra lại.");
    }
  };
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Không cần reload trang, onSnapshot sẽ tự cập nhật giao diện
    } catch (error) {
  ...
  // --- GUEST VIEW (Khách xem trang của chính mình -> Yêu cầu đăng nhập) ---
    ...
    <div className="pb-24 md:pb-10 animate-fade-in bg-[#F7F7F5] min-h-screen">
          ...
          <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 p-6 flex flex-col items-center md:flex-row md:items-end gap-4 border border-white/50">
            <div className="relative -mt-16 md:-mt-12 group">
              <div className="p-1.5 bg-white rounded-full shadow-sm">
                <img src={profileData.avatar} alt={profileData.name} className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover border-2 border-gray-50 bg-gray-100"/>
              </div>
              {profileData.isExpert && <div className="absolute bottom-1 right-1 bg-blue-500 text-white p-1.5 rounded-full border-4 border-white shadow-sm"><ShieldCheck size={16} /></div>}
            </div>
            ...
            <div className="text-center md:text-left flex-1 space-y-2">
               <div>
                    <h1 className="text-2xl font-bold text-textDark flex items-center justify-center md:justify-start gap-2">
                        {profileData.name}
                        {profileData.isExpert && <Badge text="Chuyên gia" color="blue" />}
                        {profileData.isAdmin && <Badge text="Admin" color="red" />}
                    </h1>
                    {profileData.specialty && <div className="flex items-center justify-center md:justify-start gap-1.5 mt-1 text-xs text-blue-600 font-bold bg-blue-50 px-2 py-0.5 rounded-lg inline-flex"><Briefcase size={12} /> {profileData.specialty}</div>}
               </div>
               
               <p className="text-textGray text-sm max-w-md mx-auto md:mx-0 leading-relaxed">{profileData.bio || "Thành viên tích cực của cộng đồng Asking.vn"}</p>
               
               <div className="flex items-center justify-center md:justify-start gap-4 text-sm">
                  <div className="flex items-center gap-1 font-medium text-textDark">
                      <strong className="text-lg">{profileData.followers?.length || 0}</strong> <span className="text-gray-400">Người theo dõi</span>
                  </div>
                  <div className="flex items-center gap-1 font-medium text-textDark">
                      <strong className="text-lg">{profileData.following?.length || 0}</strong> <span className="text-gray-400">Đang theo dõi</span>
                  </div>
               </div>
            </div>
      ...
      <div className="px-4 max-w-5xl mx-auto space-y-6">
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2 -mx-4 px-4 snap-x">
          ...
          <StatCard icon={<Users className="text-purple-500" size={20} />} value={profileData.followers?.length || 0} label="Followers" bg="bg-purple-50" />
        </div>
