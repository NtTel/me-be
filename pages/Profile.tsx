...
export const Profile: React.FC<ProfileProps> = ({ user, questions, onLogout, onOpenAuth }) => {
	...
	useEffect(() => {
		...
		const fetchProfile = async () => {
			...
			// Sử dụng regex đơn giản để tránh query username bằng ID
	...
	useEffect(() => {
		// Chỉ chạy khi user đã đăng nhập VÀ đang xem profile người khác
		if (user && !user.isGuest && profileData && user.id !== profileData.id) {
			// Lắng nghe trên tài khoản của CHÍNH MÌNH (user.id) để kiểm tra mảng 'following'
			const unsub = onSnapshot(doc(db, 'users', user.id), (docSnap) => {
				if (docSnap.exists()) {
					const userData = docSnap.data();
					// Kiểm tra xem ID của profile đang xem có trong danh sách following của mình không
	...
	useEffect(() => {
		// Ngăn chặn chuyển hướng nếu đang loading hoặc profile không tồn tại
		...
		if (profileData.username && currentSlug !== canonicalSlug) {
			// Chuyển hướng chỉ khi userId hiện tại không phải là ID gốc (để giữ đường dẫn ID là dự phòng)
	...
	const handleFollowToggle = async () => {
		...
		try {
			if (isFollowing) await unfollowUser(user.id, profileData.id);
				...
				// Đảm bảo user.id là ID thực, không phải slug
	...
	const handleSaveProfile = async () => {
		...
		try {
			...
			// 2. CHECK TRÙNG LẶP CHỈ KHI USERNAME THAY ĐỔI VÀ KHÔNG RỖNG
			if (finalUsername && finalUsername !== currentUsername) {
				...
				// Đảm bảo không tìm thấy bất kỳ tài liệu nào
				if (!snapshot.empty) {
			...
			await updateDoc(doc(db, 'users', profileData.id), {
				...
				username: finalUsername || null, // Lưu null nếu người dùng để trống
				coverUrl: editForm.coverUrl || null
			});

			// 4. CẬP NHẬT URL sau khi lưu thành công (Nếu username thay đổi)
	...
	// --- LOGIC ĐĂNG XUẤT (ĐÃ FIX ASYNC RACE CONDITION) ---
	const handleAuthAction = async () => {
		if (user.isGuest) {
			onOpenAuth();
		} else {
			try {
				// PHẢI AWAIT để chờ xóa token/context xong mới chuyển trang
				await onLogout(); 
				navigate('/');
			} catch (error) {
	...
	// --- RENDER VIEWS (KHÔNG THAY ĐỔI CẤU TRÚC) ---
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in no-scrollbar overflow-y-auto">
					<div className="bg-white rounded-2xl shadow-2xl w-full max-w-md my-auto animate-pop-in relative flex flex-col max-h-[90vh]">
						...
						<div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
							...
							<div>
								<label className="block text-xs font-bold text-gray-500 uppercase mb-1">Username (ID)</label>
								<div className="flex items-center relative">
									<span className="absolute left-4 text-gray-400 font-bold"><AtSign size={16}/></span>
									<input 
										type="text" 
										value={editForm.username} 
										// Loại bỏ ký tự không hợp lệ ngay khi nhập
										onChange={e => setEditForm({...editForm, username: e.target.value.toLowerCase().replace(/[^a-z0-9._]/g, '')})} 
										placeholder="nguyenvanan.99" 
										className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-primary/20 font-medium text-sm text-gray-700" 
									/>
								</div>
							</div>
