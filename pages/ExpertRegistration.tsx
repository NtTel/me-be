  ...
  if (currentUser.expertStatus === 'pending') {
    return (
      <div className="max-w-2xl mx-auto px-4 py-12 animate-fade-in">
        <div className="bg-white rounded-[2rem] shadow-lg p-8 md:p-12 text-center border border-gray-100">
          ...
          <h2 className="text-2xl font-bold text-textDark mb-4">Hồ sơ đang được xét duyệt</h2>
          <p className="text-textGray mb-8 leading-relaxed">
          ...
          <div className="bg-gray-50 rounded-xl p-6 text-left max-w-md mx-auto">
             <h4 className="font-bold text-sm text-textGray uppercase mb-4 tracking-wider">Tiến trình</h4>
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <div className="text-center mb-10">
        <h1 className="text-3xl md:text-4xl font-bold text-textDark mb-4">
          Đăng ký đối tác <span className="text-blue-600">Chuyên gia</span>
        </h1>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-4">
          ...
          <div className="bg-blue-50 p-6 rounded-2xl border border-blue-100">
             <h3 className="font-bold text-blue-800 mb-2 text-sm">Tại sao cần xác thực?</h3>
        ...
        <div className="md:col-span-2">
          <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] shadow-lg border border-gray-100 overflow-hidden">
            <div className="p-8">
              <h3 className="text-xl font-bold text-textDark mb-6">Thông tin đăng ký</h3>
              
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-bold text-textDark mb-2">Họ và tên</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-sm font-bold text-textDark mb-2">Số điện thoại</label>
                    <input 
                      type="tel" 
                ...
                <div>
                   <label className="block text-sm font-bold text-textDark mb-2">Lĩnh vực chuyên môn</label>
                      ...
                      {SPECIALTIES.map(spec => (
                        <button
                          ...
                          className={`p-3 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all ${
                             ...
                             : 'border-gray-100 hover:border-blue-200 text-textGray'
                          }`}
                        >
                           {spec.icon}
                ...
                <div>
                  <label className="block text-sm font-bold text-textDark mb-2">Đơn vị công tác hiện tại</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-sm font-bold text-textDark mb-2">Minh chứng chuyên môn</label>
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:bg-gray-50 transition-colors relative">
                    ...
                    <p className="text-sm font-medium text-textDark">Tải lên ảnh Bằng cấp / Chứng chỉ / Thẻ nhân viên</p>
                    <p className="text-xs text-textGray mt-1">Hỗ trợ JPG, PNG, PDF (Tối đa 5MB)</p>
                    {formData.files.length > 0 && (
                      <div className="mt-4 flex flex-wrap gap-2 justify-center">
                        {formData.files.map((f, i) => (
                          <span key={i} className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-md font-medium">
                            {f.name}
                          </span>
                        ))}
                      </div>
...
const TimelineItem: React.FC<{ status: 'completed' | 'active' | 'pending'; title: string; date: string }> = ({ status, title, date }) => {
  const getIcon = () => {
    ...
    return <div className="w-2.5 h-2.5 bg-gray-300 rounded-full"></div>;
  };
  ...
  return (
    <div className="flex gap-4 mb-6 last:mb-0 relative">
      {/* Line */}
      <div className="absolute left-3.5 top-8 bottom-[-24px] w-0.5 bg-gray-100 last:hidden"></div>
      
      <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 z-10 ${getColor()}`}>
        {getIcon()}
      </div>
      <div>
        <h5 className={`font-bold text-sm ${status === 'pending' ? 'text-gray-400' : 'text-textDark'}`}>{title}</h5>
